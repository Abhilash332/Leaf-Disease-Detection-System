...
# ==========================================
# 2. CONSWINTX-LITE MODEL CLASSES
# ==========================================
...
class SwinTransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, window_size=7, shift_size=0, mlp_ratio=2., qkv_bias=True, dropout=0.1):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        self.window_size = window_size
        ...
        self.attn = WindowAttention(dim, window_size=(window_size, window_size), num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=dropout, proj_drop=dropout)
...
class SwinStage(nn.Module):
    def __init__(self, dim, depth, num_heads, window_size, input_resolution, mlp_ratio=2., downsample=None):
        super().__init__()
        effective_ws = min(window_size, input_resolution[0], input_resolution[1])
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, num_heads=num_heads, window_size=effective_ws, shift_size=0 if (i % 2 == 0) else effective_ws // 2, mlp_ratio=mlp_ratio) for i in range(depth)
        ])
...
class CustomSwinTBranch(nn.Module):
    def __init__(self, img_size=IMAGE_SIZE, patch_size=PATCH_SIZE, in_chans=3, embed_dim=EMBED_DIM, depths=DEPTHS, num_heads=NUM_HEADS, window_size=WINDOW_SIZE, mlp_ratio=MLP_RATIO):
        super().__init__()
        ...
        for i in range(len(depths)):
            ...
            self.stages.append(SwinStage(dim=stage_dim, depth=depths[i], num_heads=num_heads[i], window_size=window_size, input_resolution=stage_res, mlp_ratio=mlp_ratio, downsample=downsample))
...
class InvertedResBlock(nn.Module):
    def __init__(self, in_channels, expansion, stride, alpha, filters):
        ...
        layers = []
        if expansion != 1:
            layers.extend([nn.Conv2d(in_channels, hidden_dim, kernel_size=1, bias=False), nn.BatchNorm2d(hidden_dim, eps=1e-3, momentum=0.001), nn.ReLU6(inplace=True)])
        layers.extend([nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=stride, padding=1, groups=hidden_dim, bias=False), nn.BatchNorm2d(hidden_dim, eps=1e-3, momentum=0.001), nn.ReLU6(inplace=True)])
        layers.extend([nn.Conv2d(hidden_dim, self.pointwise_filters, kernel_size=1, bias=False), nn.BatchNorm2d(self.pointwise_filters, eps=1e-3, momentum=0.001)])
...
class HybridMobileNetSwin(nn.Module):
    def __init__(self, num_classes=NUM_CLASSES):
        ...
        self.fusion_block = InvertedResBlock(in_channels=fusion_channels, expansion=6, stride=2, alpha=1.0, filters=128)
...
# ==========================================
# 3. FASTAPI SERVER & PREDICTION LOGIC
# ==========================================
app = FastAPI()
...
MODEL_PATH = r"C:\Users\srira\Downloads\conswintx_lite_cv\conswintx_lite_cv\fold_1\best_model.pt"

print("Building model architecture and loading weights...")
try:
    # 1. Initialize the empty blueprint
    model = HybridMobileNetSwin(num_classes=38)
    
    # 2. Load the dictionary of weights
    state_dict = torch.load(MODEL_PATH, map_location=torch.device('cpu'), weights_only=False)
    
    # 3. Handle extra nesting if the notebook saved it in a dictionary format
    if 'model_state' in state_dict:
        state_dict = state_dict['model_state']
        
    # 4. Pour the weights into the blueprint
...
@app.get("/")
def read_root():
    return {"message": "Plant Disease ML Microservice is running!"}

@app.post("/predict")
async def predict_disease(file: UploadFile = File(...)):
    if model is None:
        return JSONResponse(content={"error": "Model not loaded on server"}, status_code=500)
